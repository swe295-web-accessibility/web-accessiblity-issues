...
def get_response_gl(url: str, metadata_path: str, strategy="mobile"):
    """
    get response from Google lighthouse
    :param url:  url to be tested
    :param metadata_path: path that the response to be stored
    :param strategy: mobile/desktop, default is mobile
    :return:
    """
    while True:
        r = requests.get(
        ...
        if not r.json().__contains__('error'):
            break
        time.sleep(2)  # sometimes gl will return 500
    with open(metadata_path, 'w') as f1:
...
def get_info_website(url: str):
    ...
    get_response_gl(url, f'./metadata/{file_name}-mobile_meta.json')
    get_response_gl(url, f'./metadata/{file_name}-desktop_meta.json', 'desktop')
    ...
    generate_comparison_mobile_desktop(f'./csv/{file_name}-mobile.csv', f'./csv/{file_name}-desktop.csv', url)


def json2csv(json_path: str, csv_file: str):
    """
    process the JSON file, store the results as CSV file
    :param json_path: json file path
    :param csv_file: csv file path (to be stored)
    :return:
    """
    df_data = {
        "type": [],
    ...
    problem = json.load(open(json_path, 'r'))['problem']
...
def calculate_overall(metadata_path: str, processed_path: str):
    """
    reduce the metadata and store the useful information
    :param metadata_path: metadata file path
    :param processed_path: processed file path (to be stored)
    :return:
    """
        ...
        if v['scoreDisplayMode'] == 'notApplicable':
            ...
            for k1, v1 in v.items():
                if k1 == 'details':
                    ...
                    break
            with open(f"{processed_path.replace('json', 'jpg').replace('processed', 'screenshot')}", 'wb') as fp:
...
def path2XPath(path_str: str):
    """
    change the it['node']['path'] to absolute XPath
    :param path_str: path string, like HTML,1,BODY,1,DIV,1
    :return: absolute XPath
    """
    split_str = path_str.split(",")
    xpath = ""
...
def generate_comparison_mobile_desktop(mobile_file_path: str, desktop_file_path: str, url: str):
    """
    generate overview report of comparison of mobile and desktop data, for visualization use
    :param mobile_file_path: mobile file path
    :param desktop_file_path: desktop file path
    :param url: url of this page
    :return:
    """
    df1 = pd.read_csv(mobile_file_path)
    df2 = pd.read_csv(desktop_file_path)
...
def base64_to_image(base64_string: str):
    """
    turn base 64 string to image
    :param base64_string: base 64 string of one image
    :return: string after being decoded
    """
